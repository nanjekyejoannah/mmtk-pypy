...
use crate::{PyPy, PyPyEdge, UPCALLS};
use mmtk::scheduler::*;
...
macro_rules! scan_roots_work {
    ($struct_name: ident, $func_name: ident) => {
        pub struct $struct_name<F: RootsWorkFactory<PyPyEdge>> {
            factory: F,
        }

        impl<F: RootsWorkFactory<PyPyEdge>> $struct_name<F> {
            pub fn new(factory: F) -> Self {
                Self { factory }
            }
        }

        impl<F: RootsWorkFactory<PyPyEdge>> GCWork<PyPy> for $struct_name<F> {
            fn do_work(&mut self, _worker: &mut GCWorker<PyPy>, _mmtk: &'static MMTK<PyPy>) {
                unsafe {
...
pub struct ScanCodeCacheRoots<F: RootsWorkFactory<PyPyEdge>> {
    factory: F,
}

impl<F: RootsWorkFactory<PyPyEdge>> ScanCodeCacheRoots<F> {
    pub fn new(factory: F) -> Self {
        Self { factory }
    }
}

impl<F: RootsWorkFactory<PyPyEdge>> GCWork<PyPy> for ScanCodeCacheRoots<F> {
    fn do_work(&mut self, _worker: &mut GCWorker<PyPy>, _mmtk: &'static MMTK<PyPy>) {
